...
function ResultsPageClient({ storedAnswers }: { storedAnswers: string[] | null }) {
  ...
  useEffect(() => {
    ...
    const fetchResults = async () => {
      if (storedAnswers && storedAnswers.length > 0) {
        try {
          const analysisData = await getAnalysisResults(storedAnswers);
          if (analysisData.error) {
            setError(analysisData.error);
          } else {
            setResults(analysisData);
            gtmEvent('analysis_success', {
              diagnosis: analysisData.diagnosis.substring(0, 100),
            });
          }
        } catch (e) {
          setError('Ocorreu um erro ao buscar sua análise.');
          gtmEvent('analysis_failed', {
            error: (e as Error).message,
          });
        }
      } else {
         setError('Não foi possível carregar as respostas do quiz para análise.');
      }
    };
  ...
  if (error) {
    return (
        <div className="flex flex-col min-h-screen items-center justify-center text-center p-4">
            <h1 className="text-2xl font-bold text-destructive mb-4">Erro na Análise</h1>
...
export default function ResultadoPage() {
    ...
    // In a real app, you might want to handle the case where answers are not in localStorage.
    // For this implementation, we'll retrieve them and pass to the client component.
    const storedAnswers = typeof window !== 'undefined' ?
      JSON.parse(sessionStorage.getItem('quizAnswers') || '[]') :
      (answersParam ? answersParam.split(',') : []);

    return (
        <Suspense fallback={<Loading />}>
            <ResultsPageClient storedAnswers={storedAnswers} />
        </Suspense>
    );
}
